...
class BlinnPhongShader(ShaderBase):
    """
    ...
    def forward(self, fragments: Fragments, meshes: Meshes, **kwargs) -> torch.Tensor:
        ...
        # texels = meshes.sample_textures(fragments) # same quality, but faster, so XD
        lights = kwargs.get("lights", self.lights)
        materials = kwargs.get("materials", self.materials)
        texels = meshes.sample_textures(fragments)
        blend_params = kwargs.get("blend_params", self.blend_params)
        colors,pixel_normal,vindex = blinnphong_shading(
            meshes=meshes,
            fragments=fragments,
            lights=lights,
            cameras=cameras,
...
class CookTorranceShader(ShaderBase):
    """
    Per pixel lighting - the lighting model is applied using the interpolated
    coordinates and normals for each pixel. The blending function hard assigns
    the color of the closest face for each pixel.
    ...
    def forward(self, fragments: Fragments, meshes: Meshes, **kwargs) -> torch.Tensor:
        ...
        texels = meshes.sample_textures(fragments)
        blend_params = kwargs.get("blend_params", self.blend_params)
        colors,pixel_normal,vindex = cooktorrance_shading(
            meshes=meshes,
            fragments=fragments,
            lights=lights,
            cameras=cameras,
...
class HardPhongShader(ShaderBase):
    ...
    def forward(self, fragments: Fragments, meshes: Meshes, **kwargs) -> torch.Tensor:
        ...
        texels = meshes.sample_textures(fragments)
        blend_params = kwargs.get("blend_params", self.blend_params)
        colors,pixel_normal,vindex = phong_shading(
            meshes=meshes,
            fragments=fragments,
            lights=lights,
            cameras=cameras,
